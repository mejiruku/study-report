...
function saveViewModePreference(viewMode) {
    if (currentUser) {
        db.collection("users").doc(currentUser.uid).set({
            ...
            merge: true
        }).then(() => console.log("View mode saved to cloud")).catch((err) => console.error("Failed to save view mode", err));
    } else {
    ...
    if (currentUser) {
        try {
            ...
            if (doc.exists) {
                ...
                window._cachedSpecialCodeEnabled = doc.data().specialCodeEnabled === true;
            } else {
        ...
        for (const log of logs) {
            ...
            if (newLog.createdAt && typeof newLog.createdAt === "number") {
                try {
                    ...
                    if (isNaN(newLog.createdAt.getTime())) throw new Error("Invalid Date");
                } catch (e) {
...
async function syncDataOnLogin() {
    ...
    try {
        ...
        const {
            toUpload,
            toDownload
        } = compareAndMerge(localData, cloudData);
...
async function uploadToCloud(dataToUpload) {
    ...
    Object.keys(dataToUpload).forEach((dateKey) => {
        ...
        promises.push(reportsRef.doc(dateKey).set({
